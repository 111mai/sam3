...
构建车辆分类数据集
==================
功能：从 SAM3 标签（.npz）中提取车辆目标，裁剪后按类别存入 ImageFolder 结构，供 train_cls.py 训练使用。
- 输入：images/ 图片 + sam3_label/ .npz 标签文件
- 输出：cls_dataset/ 目录，按类别分子文件夹（construction vehicle / truck / car / bus）
- 过滤条件：置信度 ≥ 0.9、mask 面积 ≥ 256px、每类最多 5000 张
- 每张目标图：根据 mask 裁剪 → 贴白底 → 保持前景占比 85% → 方形 RGB 图
"""
